...
devstack 설정을 위한 local.conf 파일을 생성한다. 하기 예제 local.conf에서 IP 정보만 변경한다.
```
[[local|localrc]]
HOST_IP=10.1.1.26
...
enable_plugin networking-onos https://github.com/gunine/networking-onos.git stable/ocata
ONOS_MODE=allinone
...
stack.sh 쉘 스크립트를 실행하여 OpenStack을 설치한다.
```
$ cd ~/devstack
$ ./stack.sh
```
...
* Network Name: net1, net2
* Subnet Name: snet1, snet2
* CIDR: 20.1.1.0/24, 20.2.2.0/24
* Gateway: 20.1.1.1, 20.2.2.1
